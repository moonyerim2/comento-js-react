import { RAW_PRODUCTS as products } from "../../resources/products.js";
...
const typo = document.getElementById("backgroundTypo");
const collections = document.getElementById("collectionProducts");
const seasonal = document.getElementById("seasonalProducts");
typo.textContent = TYPO_TEXT;
...
const createProductCard = ({ id, nameI18n, image, name, description }) => `
  <div id=${`product${id}`} class="product-card">
    <div class="flex-1">
      <img src="${image}" alt="${name}" style="width: 100%; height: 100%; object-fit: fill;">
    </div>
...
const productsProxy = new Proxy(products, {
  get: (target, prop) => {
    return target
      .filter((product) => product.category === prop)
      .map(createProductCard)
      .join("");
  },
});

collections.innerHTML = productsProxy.collection;
seasonal.innerHTML = productsProxy.seasonal;
